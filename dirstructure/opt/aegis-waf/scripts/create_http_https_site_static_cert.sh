...
echo "Paste SSL Certificate and Certificate Chain including -----BEGIN CERTIFICATE----- and -----END CERTIFICATE----- entries followed by a blank line:"
PEM=$(sed '/^$/q')
echo "Paste SSL Unencrypted Key including -----BEGIN RSA PRIVATE KEY----- and -----END RSA PRIVATE KEY----- entries followed by a blank line:"
KEY=$(sed '/^$/q')
...
#CREATE WEBSITE NGINX CONFIG FILE
/bin/cp /opt/aegis-waf/templates/https_template_site.conf /usr/local/nginx/conf/sites-available/${SITE}-ssl.conf
...
echo "Creating Site SSL Certificate File"
/bin/cp /opt/aegis-waf/templates/certificate_template.pem /usr/local/nginx/conf/ssl/${SITE}.pem
...
echo "Creating Site SSL Certificate Key File"
/bin/cp /opt/aegis-waf/templates/key_template.pem /usr/local/nginx/conf/ssl/${SITE}.key
